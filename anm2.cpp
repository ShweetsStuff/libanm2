...
XMLError query_string_attribute(XMLElement *element, const char *attribute,
                                std::string *value) {
  const char *temp = nullptr;
  auto result = element->QueryStringAttribute(attribute, &temp);
  if (result == XML_SUCCESS && temp && value)
    *value = temp;
  return result;
}

XMLError query_path_attribute(XMLElement *element, const char *attribute,
...
XMLError query_color_attribute(XMLElement *element, const char *attribute,
                               float *value) {
  int temp{};
  auto result = element->QueryIntAttribute(attribute, &temp);
  if (result == XML_SUCCESS && value)
    *value = (temp / 255.0f);
  return result;
}
...
Info::Info(XMLElement *element) {
  if (!element)
    return;
  query_string_attribute(element, "CreatedBy", &createdBy);
  query_string_attribute(element, "CreatedOn", &createdOn);
...
Sound::Sound(XMLElement *element, int &id, SoundCallback soundCallback) {
  ...
  query_path_attribute(element, "Path", &path);
  if (soundCallback)
    userData = soundCallback(path);
}

Content::Content(XMLElement *element, TextureCallback textureCallback,
                 SoundCallback soundCallback) {
  if (auto spritesheetsElement = element->FirstChildElement("Spritesheets")) {
    for (auto child = spritesheetsElement->FirstChildElement("Spritesheet");
         child; child = child->NextSiblingElement("Spritesheet")) {
      int spritesheetId{};
  ...
  if (auto nullsElement = element->FirstChildElement("Nulls")) {
    for (auto child = nullsElement->FirstChildElement("Null"); child;
...
Item::Item(XMLElement *element, Type type, int &id) {
  if (type == LAYER)
    element->QueryIntAttribute("LayerId", &id);
  if (type == NULL_)
    element->QueryIntAttribute("NullId", &id);

  element->QueryBoolAttribute("Visible", &isVisible);

  for (auto child = type == TRIGGER ? element->FirstChildElement("Trigger")
                                    : element->FirstChildElement("Frame");
       child; child = type == TRIGGER ? child->NextSiblingElement("Trigger")
                                      : child->NextSiblingElement("Frame"))
...
Animation::Animation(XMLElement *element) {
  ...
  if (auto layerAnimationsElement =
          element->FirstChildElement("LayerAnimations")) {
    for (auto child =
             layerAnimationsElement->FirstChildElement("LayerAnimation");
         child; child = child->NextSiblingElement("LayerAnimation")) {
      int layerId{};
  ...
  if (auto nullAnimationsElement =
          element->FirstChildElement("NullAnimations")) {
    for (auto child = nullAnimationsElement->FirstChildElement("NullAnimation");
         child; child = child->NextSiblingElement("NullAnimation")) {
      int nullId{};
...
Animations::Animations(XMLElement *element) {
  ...
  for (auto child = element->FirstChildElement("Animation"); child;
       child = child->NextSiblingElement("Animation"))
    items.emplace_back(Animation(child));
}

Anm2::Anm2(const std::filesystem::path &path, TextureCallback textureCallback,
           SoundCallback soundCallback) {
  XMLDocument document;

  if (document.LoadFile(path.c_str()) != XML_SUCCESS) {
    std::cout << document.ErrorStr();
    return;
  }

  WorkingDirectory workingDirectory(path.parent_path());

  auto element = document.RootElement();

  if (auto infoElement = element->FirstChildElement("Info"))
    info = Info(infoElement);
  ...
  if (auto animationsElement = element->FirstChildElement("Animations"))
    animations = Animations(animationsElement);
}
} // namespace anm2
